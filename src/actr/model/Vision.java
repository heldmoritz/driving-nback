...
public class Vision extends Module {
	...
	private class VisualObject {
		...
		public String toString() {
			return "(" + id + " kind " + kind + " screen-x " + x + " screen-y " + y + " value " + value + " :attended "
					+ (attended ? "t" : "nil") + " :noticed " + (noticed ? "t" : "nil") + ")";
		}
	}
	...
	/**
	 * Adds a visual object for access by the vision module. The identifier should
	 * be unique for different sounds; if the same identifier is used, the new
	 * object will cancel the previous one (as might be useful for the movements of
	 * a single object).
	 * 
	 * @param id    the unique name of the visual object
	 * @param type  the type of the visual object (i.e., the "kind" slot value)
	 * @param value the value of the visual object (i.e., the "value" slot value)
	 * @param x     the x coordinate of the visual object (i.e., the "screen-x" slot
	 *              value)
	 * @param y     the y coordinate of the visual object (i.e., the "screen-y" slot
	 *              value)
	 * @param w     the width of the visual object (i.e., the "width" slot value)
	 * @param h     the height of the visual object (i.e., the "height" slot value)
	 * @param d     the distance of the visual object (i.e., the "distance" slot
	 *              value)
	 */
	...
	/**
	 * Adds a visual object for access by the vision module. The identifier should
	 * be unique for different objects; if the same identifier is used, the new
	 * object will cancel the previous one (as might be useful for the movements of
	 * a single object). Using this helper method, the object will not be stuffed
	 * into the visicon even if buffer stuffing is enabled!
	 * 
	 * @param id    the unique name of the visual object
	 * @param type  the type of the visual object (i.e., the "kind" slot value)
	 * @param value the value of the visual object (i.e., the "value" slot value)
	 ...
	 *              value)
	 */
	public void addVisualnoStuffing(String id, String type, String value, int x, int y, int w, int h, int d) {
		VisualObject vo = new VisualObject(Symbol.get(id), Symbol.get(type), Symbol.get(value), x, y, w, h, d);
		visicon.put(Symbol.get(id), vo);
		// model.noteTaskUpdated();
	}
	...
	/**
	 * Changes a visual object in the visual scene. A method to model object
	 * consistency with changing values of the object.
	 * 
	 * @author Moritz Held
	 * @param id    the unique name of the visual object
	 * @param value the value of the visual object (i.e., the "value" slot value)
	...
	public void changeValue(String id, String value) {
		...
		if (vo != null)
			vo.value = Symbol.get(value);
		model.noteTaskUpdated();
	}

	/**
	 * Enables the modeler to make use of buffer stuffing in task with a high amount
	 * of visual-location requests that may cause new visual objects to be
	 * overlooked.
	 * 
	 */
	...
	public void queuedStuffing() {
		...
		while (it.hasNext()) {
			...
			if ((model.getTime() - vo.creationTime) < model.qs && vo.creationTime != 0) {
				if (model.bufferStuffing && vo.noticed == false && (model.getBuffers().get(Symbol.visloc) == null
						&& model.getBuffers().get(Symbol.vislocState).get(Symbol.buffer) == Symbol.empty)) {
					double newDist = Utilities.distance(vo.x, vo.y, lastVisLocRequestX, lastVisLocRequestY);
					double curDist = 99999;
					try {
						Chunk curvisloc = model.getBuffers().get(Symbol.visloc);
	 ...
	 * @param y  the y coordinate of the visual object (i.e., the "screen-y" slot
	 *           value)
	 * @param d  the distance of the visual object (i.e., the "distance" slot value)
	 */
	...
	void setVisualFrequency(String string, double frequency) {
		frequencies.put(string, Double.valueOf(frequency));
	}
	...
	private Chunk findVisualLocation(Chunk request, Iterator<VisualObject> it) {
		...
		while (it.hasNext()) {
			VisualObject vo = it.next();
			if (matchesVisualObject(request, vo))
				found.add(vo);
		}
		if (found.isEmpty())
			return null;
	...
	void update() {
		...
		if (request != null && request.isRequest() && request.get(Symbol.isa) == Symbol.get("move-attention")) {
			...
			double encodingTime = (useEMMA) ? computeEncodingTime(vo) : visualAttentionLatency;
	...
	private void prepareEyeMovement(final VisualObject vo, final Chunk visual) {
		...
		model.addEvent(new Event(model.getTime() + emmaPreparationTime, "eye",
	...
	private void executeEyeMovement(final VisualObject vo, final Chunk visual) {
		double executionTime = emmaExecutionBaseTime + emmaExecutionTimeIncrement * computeEccentricity(vo);
